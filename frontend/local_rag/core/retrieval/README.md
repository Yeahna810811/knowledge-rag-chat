...
100 题 benchmark（80 可答）、70 chunk 语料（8 篇评测语料 + 60 个难负样本 chunk，
噪声取自本项目自身的真实文档）、真实 bge-small-zh 向量，配对 bootstrap 10000 轮，
对照 bm25 单路：

| 对比 | ΔMRR（原始问法） | 95% CI | p | ΔMRR（口语化问法） | 结论 |
|---|---|---|---|---|---|
| dense | −0.2317 | [−0.3344, −0.1313] | 0.000 | −0.1642（p=0.001） | **显著更差** |
| hybrid 1:1 | −0.1227 | [−0.2050, −0.0446] | 0.002 | −0.0108（不显著） | **不涨点，且原始问法下更差** |
| hybrid 2:1 | −0.1050 | [−0.1740, −0.0435] | 0.001 | +0.0150（不显著） | 同上 |

结论：**BM25 显著优于稠密路**（两种问法都显著）；
**混合检索在任何权重下都没有显著收益**，因此不值得为它多维护一套索引和调参。

口语化问法下 BM25 的 MRR 从 0.908 掉到 0.462（稠密路同样腰斩到 0.298），
...
70 chunk、50 条查询（`python evaluation/benchmark_runtime.py`）：

| 指标 | BM25 | 稠密 | 倍数 |
|---|---|---|---|
| 冷启动（加载模型） | 0 ms | 367.42 ms | — |
| 入库 70 chunk | 7.82 ms | 7071.50 ms | 904× |
| 单条查询 | 0.09 ms | 19.32 ms | 216× |
...
冷启动 367ms 还是权重已在缓存的情况，首次部署另需下载约 95MB 模型。
