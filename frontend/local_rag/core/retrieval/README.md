...
100 题 benchmark（80 可答）、45 chunk 语料（8 篇评测语料 + 35 个难负样本 chunk，
噪声取自本项目自身的真实文档）、真实 bge-small-zh 向量，配对 bootstrap 10000 轮，
对照 bm25 单路：

| 对比 | ΔMRR（原始问法） | 95% CI | p | ΔMRR（口语化问法） | 结论 |
|---|---|---|---|---|---|
| dense | −0.1958 | [−0.2990, −0.0933] | 0.000 | −0.1973（p=0.000） | **显著更差** |
| hybrid 1:1 | −0.0973 | [−0.1765, −0.0210] | 0.013 | −0.0392（不显著） | **不涨点，且原始问法下更差** |
| hybrid 2:1 | −0.0694 | [−0.1377, −0.0060] | 0.031 | +0.0148（不显著） | 同上 |

结论：**BM25 显著优于稠密路**（两种问法都显著）；
**混合检索在任何权重下都没有显著收益**，因此不值得为它多维护一套索引和调参。

口语化问法下 BM25 的 MRR 从 0.893 掉到 0.510（稠密路同样腰斩到 0.313），
说明真正的瓶颈是**词汇不匹配**——下一步该做查询改写 / HyDE，
而不是继续调检索器权重。
...
45 chunk、45 条查询（`python evaluation/benchmark_runtime.py`）：

| 指标 | BM25 | 稠密 | 倍数 |
|---|---|---|---|
| 冷启动（加载模型） | 0 ms | 332.49 ms | — |
| 入库 45 chunk | 3.30 ms | 4920.37 ms | 1490× |
| 单条查询 | 0.05 ms | 21.26 ms | 400× |
...
冷启动 332ms 还是权重已在缓存的情况，首次部署另需下载约 95MB 模型。
